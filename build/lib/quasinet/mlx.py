...
def get_terminal_distinction_coeff(terminal_class_vector,
    ...
    for node in  terminal_node_list:
        thisvalue=0
        for class__,value__ in list(terminal_class_vector[node].items()):
...
def feature_importance(significant_dec_path,
    ...
    for key__,path__ in list(significant_dec_path.items()):
        old_num=0
        augmented_path=[]
...
def visTree(MODEL,PR,PLOT=True,VERBOSE=False,
    ...
    tprob_significant={key__:tval for key__,tval in list(tprob.items())
                       if tval >= PROB_MIN}
    ...
    for node in ndid(MODEL):
        ...
        class_vector_[node]=dict(list(zip(A.names[0],list(A))))

        A=sapply(nodeapply(assimpleparty(MODEL),ids = node,
                           FUN=infonode),criteria="error")
        A=dict(list(zip(A.names[0].replace(str(node),'E'),list(A))))
        error_[node]=A['E']

        A=sapply(nodeapply(assimpleparty(MODEL), ids = node,
                           FUN=infonode),criteria="n")
        A=dict(list(zip(A.names[0].replace(str(node),'N'),list(A))))
        num_pass_[node]=A['N']
    ...
    for rl in RLS_:
        ...
        if ID[count] in list(tprob_significant.keys()):
...
def setdataframe(file1,outname="",
    ...
    D1=pd.read_csv(file1,delimiter=",",index_col=None,
                   engine='python')
    D1.columns = list(map(nameclean,D1.columns.values))
...
def getresponseframe(DATA,MODEL,RESPONSE_,olddata=False):
    ...
    data = list(map(list, list(zip(*list(A.keys()))))) + [list(A.values())]
    cf = pd.DataFrame(list(zip(*data))).set_index([0, 1])[2].unstack()
...
def tree_export(TR,outfilename='out.dot',
    ...
    node_color={}
    for node_id in list(TR.class_pred_.keys()):
    ...
    node_str={}
    for node_id in list(TR.feature.keys()):
        STR=""
    ...
    for parent in list(TR.children.keys()):
...
def dictprod(dict_,a=1.0):
    ...
    '''
    return {key:value*a for (key,value) in dict_.items()}
...
def normalizedict(dict_):
    ...
    for key in list(dict_.keys()):
        s=s+dict_[key]
    return {key:(value/s) for (key,value) in dict_.items()}
...
def mergedistributions(dist_):
    '''
        ...
        all component dicts
    '''
    num=len(list(dist_.keys()))
    key_list=[]
    for key in list(dist_.keys()):
        key_list=np.append(key_list,list(dist_[key].keys()))
        
    D={}
    for key in key_list:
        D[key]=0.0
        
    for count in list(dist_.keys()):
        for key_ in list(dist_[count].keys()):
            if key_ in dist_[count]:
                D[key_]=D[key_]+dist_[count][key_]
    return {key:value/(num+0.0) for (key,value) in D.items() }
...
def getMergedDistribution(tree,cond={}):
    ...
    node_id_map={feature_name:np.array([], dtype=int)
                 for (i,feature_name) in tree.feature.items()}
    for (i,feature_name) in tree.feature.items():
    ...
    children={i:set() for i in list(cond.keys())}
    for feature_name in list(cond.keys()):
...
def sampleTree(tree,cond={},sample='mle',DIST=False,NUMSAMPLE=10):
    ...
    if sample is 'mle':
        sample=max(iter(dist_.items()), key=operator.itemgetter(1))[0]
    if sample is 'random':
        probs = list(dist_.values())
        keys =  list(dist_.keys())
...
def getFmap(PATH_TO_TREES):
    ...
    for filename in TREES:
        ...
        F[index]=[]
        TREE[index]=TR
        for key,value in TR.feature.items():
...
def qDistance(seq0,seq1,PATH_TO_TREES):
    ...
    for key0 in list(P0.keys()):
        if key0 in list(P1.keys()):
