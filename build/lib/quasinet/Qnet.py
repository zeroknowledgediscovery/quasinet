...
def processEdgeUpdate(edges_):
	SOURCES_=[i[0] for  i in list(edges_.keys()) if edges_[i]>0.0]
	PROCESSED_=list(set([i[1] for  i in list(edges_.keys())]))
...
def apply_threshold_to_tree(args):
	...
	sorted_feature_imp\
	= sorted(list(TR.significant_feature_weight_.items()),
