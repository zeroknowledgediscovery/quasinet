...
class Qnet(object):
    ...
    def average_fidelity(self, s=None, eps=1e-12):
        """Compute the average fidelity of a model’s predicted distributions.

        For each feature, the model predicts a probability distribution over
        possible values. Given the observed value for that feature, this function
        computes the ratio of:
            - the predicted probability assigned to the observed value, to
            - the maximum predicted probability across all possible values
              for that feature.

        If the observed value is not present in the distribution, its probability
        is imputed as the maximum probability of that distribution. Ratios are
        thus always in the interval [0, 1].
        ...
        model : object
            ...
            - ``predict_distributions(s)``: returns a list of dictionaries, each
              mapping feature values (keys) to predicted probabilities (values).
            - ``feature_names``: used to determine the number of features.
        s : list, optional
            Observed values for each feature. If ``None``, a default set of values
            is generated by ``nulldist(model)``.
        eps : float, optional
            Small constant to avoid division by zero if a distribution is empty
            or has maximum probability 0. Default is 1e-12.

        Returns
        -------
        mean_fidelity : float
            The average ratio across all features, i.e., the overall fidelity.
        ratios : numpy.ndarray
            Array of per-feature ratios (observed probability / max probability).
        """
        if s is None:
            s = getNull(self)
...
def qdistance(seq1, seq2, qnet1, qnet2, mismatch=False, FULL_C=False):
...
def qdistance_matrix(seqs1, seqs2, qnet1, qnet2):
    ...
    if symmetric:
        ...
        distance_matrix = theta_matrix(seqs1_distribs)
    else:
